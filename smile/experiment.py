...
class Experiment(object):
    ...
    *Experiment* variable is to **set** and **get** variables during
    ...
    def reserve_data_filename(self, title, ext=None, use_timestamp=False):
        ...
        disambiguate data files with the same title, extension, and
        time-stamp.  The filename is not a file path.  The filename will
        be distinct from all filenames previously returned from this method
        even if a file of that name has not yet been created in the log
        directory.
        ...
        with self._reserved_data_filenames_lock:
            ...
            for distinguisher in xrange(256):
                ...
                raise RuntimeError("Too many data files with the same "
                                   "title, extension, and timestamp!")
