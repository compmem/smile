...
class Grating(Widget):
    ...
    repeats the texture 3 times in the top left, top right, and bottom left
    quadrants'''
    ...
    def _calc_mask(self, rx, ry):
        ...
        if self.envelope[0].lower() == 'g':
            transparency = math.exp(-0.5 * (dy / (self.std_dev * 3)) ** 2 -
                                    0.5 * (dx / (self.std_dev * 3)) ** 2)
    ...
    def _calc_color(self, x):
        ...
        amp = \
            0.5 + 0.5 * \
            math.sin((x * math.pi / 180) * self.frequency + self.phase)
        # RGB color return
    ...
    and bottom left quadrant to increase efficiency. Also it repeats the sin
    wave, created in the  _calc_color function to fill the rectangle with the
    sin wave based grating.'''
    ...
    The function calls the _calc_color function to create the grating texture
    which is layered behind the mask.'''
    ...
    The function calls the mask creating function. Also, it stores masks in a
    cache, for later use to increase function efficiency.'''
