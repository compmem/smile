...
parameters passed in. You can use any of the parameters or properties that a
**Rectagnle** State has access to.
...
"""
FloatLayout.__doc__ = """A **WidgetState** where the postion of the children is
realitive to the Experiment Screen.
...
file_econdings : list (Parameter, optional, default=["utf-8", "latin1",
                       "cp1252"])
...
rootpath : string (Parameter, optional, default=None)
    Root path to use instead of the system root path. If set, it will not
    show a ".." directory to go up to the root path. For example, if you set
...
sort_func : object (Parameter, optional, defaults to a function returning
    ...
    implementation as the second argument. It
...
to be created in a box with width=200 and unlimited height:
    ...
    texture_size : list (Property)
        ...
        rescale it proportionally to fit the size of the label in order to make
        the text fit maximally in the label.
...
$ print("Hello world")
    ...
    with UntilDone():
        ...
        rect.slide(color=(0.0, 1.0, 1.0, 1.0), duration=10.0,
                   name="color fade")
        with Meanwhile():
