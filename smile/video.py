...
class WidgetState(VisualState):
    ...
    def animate(self, interval=None, duration=None, parent=None, save_log=True,
        ...
        interval : float
            ...
            the same interval as the frame-rate.  You cannot set interval to
            any number faster than the frame-rate.
        duration : float
...
For other parameters or properties that this Widget might have, refer to the
Kivy documentation for 'kivy.graphics.Rectangle.
...
parameters passed in. You can use any of the parameters or properties
that a **Rectagnle** State has access to.
...
background_disabled_down : string (Parameter, optional,
    default="atlas://data/images/defaulttheme/button_disabled_pressed")
...
background_disabled_normal : string (Parameter, optional,
    default="atlas://data/images/defaulttheme/button_disabled")
    Background image of the button used for the default graphical
...
disabled_foreground_color : list (Parameter, optional,
    default=[0.0, 0.0, 0.0, 0;5]) Current color of the foreground when
    disabled, in (r, g, b, a) format.
background_disabled_normal : string (Parameter, optional,
    default="atlas://data/images/defaulttheme/textinput_disabled")
    Background image of the TextInput when disabled.
background_normal : string (Parameter, optional,
    default="atlas://data/images/defaulttheme/textinput")
...
cursor : tuple (Property)
    ...
    will be automatically updated to ensure that the cursor is visible
    inside the viewport.
...
selection_color : list (Parameter, optional, default= [0.1843, 0.6549, 0.8313,
    .5]) Current color of the selection, in (r, g, b, a) format.
...
use_bubble : boolean (Parameter, optional, default=True on Mobile OS's, and
    False on Desktop OS's) Indicates whether the cut/copy/paste bubble is used.
use_handles : boolean (Parameter, optional, default=True on mobile OS's,
    False on desktop OS's) Indicates whether the selection handles are
    displayed.
...
This State supports all all the same features as a **TextInput** but also
allows highlighting in different programming languages.
...
background_checkbox_disabled_down : string (Parameter, optional,
    default="atlas://data/images/defaulttheme/checkbox_disabled_on")
...
"""
RelativeLayout.__doc__ = """A **WidgetState** Layout where its children are
positioned realitive to itself.
...
*swiped* around. When used in conjuction with other layouts, each layout will
be a different page.
...
scroll_distance : integer (Parameter, optional, default=20)
    ...
    touch event will go to children. It is advisable that you base this value
    on the dpi of your target device's screen.
scroll_timeout : integer (Parameter, optional, default=55)
    Timeout allowed to trigger the scroll_distance, in milliseconds. If the
    user has not moved scroll_distance within the timeout, the scrolling will
    be disabled, and the touch event will go to the children.
...
toctrees : dictionary (Property)
    ...
    when a rst document is explicitly loaded or where preload() has been
    called.
...
FileChooserListView.__doc__ = """A **WidgetState** that displays and selects
...
file_econdings : list (Parameter, optional,
    default=["utf-8", "latin1", "cp1252"]) Possible encodings for decoding a
    filename to unicode. In the case that the user has a non-ascii filename,
    undecodable without knowing it's initial encoding, we have no other choice
    than to guess it.
...
filters : list (Parameter, optional, default=[])
    ...
    strings that match the file name patteres. Use "*" to match all files, or
    "*.png" to only match png files. You can also use a sequence of characters
    to match any files with that sequence in it.
...
class Video(WidgetState.wrap(kivy.uix.video.Video)):
    ...
    allow_strech : boolean (Parameter, optional, default = False)
        ...
        image box. Otherwise, if the box is too tall, the image will
        not be stretched more than 1:1 pixels.
...
$ print("Hello world")
    ...
    with BoxLayout(width=500, height=500, top=exp.screen.top, duration=4.0):
        ...
        Rectangle(color=(0.0, 0.0, 1.0, 1.0), pos=(0, 0), size_hint=(1, 1),
                  duration=1.0)
