...
class ButtonPress(CallbackState):
    ...
    def button_callback(self):
        from .mouse import MouseWithin, MousePos
        self.claim_exceptions()
        ...
        #@FIX: Added check to see if mouse is within a button
        button=None
        pos=MousePos()
        for i in self.__buttons:
            if (MouseWithin(i, pos)):
                button=i
                break
        if button is None:
            return
        #button = Ref.cond(MouseWithin(pressed_list[0]), button, None)

        # make sure not a disabled button
        #@FIX: Changed self.__button_names to self.__buttons
        if self.__buttons[self.__buttons.index(button)].disabled.eval():
            return
