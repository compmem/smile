from video import (WidgetState, UpdateWidget, Label, ToggleButton,
                   FloatLayout, ButtonPress, Button, Rectangle, TextInput,
                   Slider)
...
def csv2loq(filename):
    ...
    formatted correctly or it will not produce the list of questions that
    you want.

    The first line of the csv are 4 columns, *question*, *ans*, *type*, and
    *multiline*.
    In the type column, you must put the type of question that you want to
    display.
            ...
            with Parallel() as fullp:
                ...
                with Serial():
                    with Loop(Ref(len, self.loq)) as lp:
                        ...
                        qfc = Func(update_dict, self.question_info_default,
                                   self.row)
                        ...
                        with If(self.q_type == "MC"):
                            ...
                            with Loop(Ref(len,
                                ...
                                # Make sure new bottom and height is updated.
                        ...
                        with If(self.q_type == "LI"):
                            ...
                            with Loop(Ref(len,
                                ...
                                # self.height_count =\
                                #   (self.height_count + mcvtb.height +
                                #    min_marg_dist)
                        ...
                        with Elif(self.q_type == "CA"):
                            ...
                            with Loop(Ref(len,
                                ...
                                # Get the current answer
                                self.cur_ca_but = Ref.getitem(
                                    self.question_info['ans'],
                                    self.ca_but_count)
                                with fullp.insert() as cainsert:
                                    # Create label to display the answer
                                    cavlb = Label(
                                        text=self.cur_ca_but,
                                        left=fl.left + 2.5 * min_marg_dist,
                                        y=self.new_bottom + min_marg_dist,
                                        text_size=((fl.width - 4 *
                                                    min_marg_dist - 2.5 *
                            ...
                            with fullp.insert() as tiinsert:
                                tif = TextInput(
                                    ...
                                    height=self.question_info[
                                        'multiline'] * def_text_input_height,
                                    save_log=False)
                        ...
                        with Elif(self.q_type == "CT"):
                            with fullp.insert() as ctinsert:
                                slf = Slider(
                                    ...
                                    save_log=False)
                                slminlbf = Label(
                                    text=self.question_info['ans'][0],
                                    left=slf.left,
                                    bottom=slf.top - 1.7 * min_marg_dist,
                                    font_size=font_size * 3 / 4,
                                    save_log=False)
                                slmidlbf = Label(
                                    text=self.question_info['ans'][1],
                                    center_x=slf.center_x,
                                    bottom=slf.top - 1.7 * min_marg_dist,
                                    font_size=font_size * 3 / 4,
                                    save_log=False)
                                slmaxlbf = Label(
                                    text=self.question_info['ans'][2],
                                    right=slf.right,
                                    bottom=slf.top - 1.7 * min_marg_dist,
                                    font_size=font_size * 3 / 4,
                                    save_log=False)
                ...
                self.results +=\
                    [{"question": question.current['question'],
                      ...
                      "answers":[{"ans": "text_input_value",
                                  "value": self.TI_refs[
                                   question.current['answers_index']].text}]}]
                ...
                self.results +=\
                    [{"question": question.current['question'],
                      ...
                      "answers":[{"ans": "slider_value",
                                  "value": self.CT_refs[
                                   question.current['answers_index']].value}]}]
            with Elif(question.current['type'] == "MC"):
                self.mc_temp_answers_list = []
                with Loop(
                    self.MC_refs[question.current[
    ...
    bob = [{'type': "TITLE",
           ...
           {'type': "CT",
            'question': ("Where left is Too much, and right is too little, "
                         "and middle is just right, how much does your car "
                         "cost?"),
            'ans': ['', '', ''],
            'max':10,
            'min':-10},
           ...
           {'type': "TI",
            'question': "If you could do anything in the world right now, "
                        "what would it be?",
            'multiline': 1},
           {'type': "TI",
