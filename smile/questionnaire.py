from video import (WidgetState, UpdateWidget, Label, ToggleButton, FloatLayout,
                   ButtonPress, Button, Rectangle, TextInput, Slider)
from state import (Loop, Parallel, If, Elif, Else, Serial, Func, UntilDone,
                   Log, Debug, Subroutine)
from ref import Ref
...
def csv2loq(filename):
    """A function that allows the Experimenter to bring in a list of questions
    from a CSV.

    From the CSV, you can pull in a list of questions.  This CSV has to be
    formatted correctly or it will not produce the list of questions that you
    want.

    The first line of the csv are 4 columns, *question*, *ans*, *type*, and
    *multiline*. In the type column, you must put the type of question that
    you want to display.
    ...
    LI : Likert
        ...
        display answers horizontally, but answers may overlap if the strings
        are too long, or if the width of the Questionnaire is too small.
        Always test to see if the likert scale is displaying how you want it
        to before administering this questionnaire on any participants.
    ...
    of each question will have all the columns that it needs filled, but for
    all the extra values needed in *ans*, you need new rows that only have
    values in the *ans* column. See yoyo.csv as an example as to what this
    would look like. Also, see the Docstring for *Questionnaire* for what
    types of questions that you need extra *ans* values for.
    ...
    while(nt['type'] is not None):
        ...
        if ((nt['type'] == "CA") or
            ...
            temp['ans'] += [nt['ans']]
            nt = reader.next()
            while((nt['type'] == "") and (nt['ans'] is not None)):
                temp['ans'] += [nt['ans']]
                try:
                    nt = reader.next()
                except:
...
def Questionnaire(self,
    ...
    loq : list of dictionaries
        ...
        display a Title anywhere in the list of questions, or in multiple
        places, to title a section of questions.
    height : integer
    ...
    *type* you pass in a string representing the type of question that you want
    ...
    with Else():
        self.y = y
    with If(width is None):
        self.width = self.exp.screen.width
    with Else():
        self.width = width
    with If(height is None):
    ...
    with ScrollView(width=self.width, height=self.height - self.button_area,
                    x=self.x, y=self.y + self.button_area) as sv:
        with FloatLayout(size=sv.size, pos=sv.pos) as fl:
            with Parallel() as fullp:
                rcf = Rectangle(color=(0.3, 0.3, 0.3, 1.0), top=fl.top,
                                left=fl.left, width=self.width,
                                height=self.height)
                ...
                with Serial():
                    with Loop(Ref(len, self.loq)) as lp:
                        ...
                        self.question_info_default =\
                            {"type": "TI", "question": "", "multiline": 1,
                        ...
                        with If(self.q_type == "MC"):
                            ...
                            with Loop(Ref(len,
                                ...
                                self.cur_tog_but =\
                                    Ref.getitem(self.question_info['ans'],
                                                self.tog_but_count)
                                with fullp.insert() as tbinsert:
                                    # Create label to display the answer
                                    mcvlb =\
                                        Label(text=self.cur_tog_but,
                                              left=(fl.left + 2.5 *
                                                    min_marg_dist),
                                              y=(self.new_bottom +
                                                 min_marg_dist),
                                              text_size=(fl.width - 4 *
                                                         min_marg_dist -
                                                         2.5 * min_marg_dist,
                                                         None),
                                              save_log=False)
                            ...
                            with fullp.insert():
                                mcrec =\
                                    Rectangle(color=(0.2, 0.2, 0.2, 1.0),
                                              left=(fl.left + 1.5 *
                                                    min_marg_dist),
                                              width=(fl.width - 3.0 *
                                                     min_marg_dist),
                                              bottom=(self.new_bottom +
                                                      min_marg_dist),
                                              save_log=False)
                        ...
                        with If(self.q_type == "LI"):
                            ...
                            # Answers from the bottom up.
                            self.li_but_count =\
                                Ref(len, self.question_info['ans'])
                            self.new_but_mid = self.width
                            self.li_width_inc =\
                                self.width / (self.li_but_count + 1)
                            ...
                            with Loop(Ref(len,
                                ...
                                # Decrement the counter
                                self.new_but_mid =\
                                    self.new_but_mid - self.li_width_inc
                                self.li_but_count = self.li_but_count - 1
                                # Get the current answer
                                self.cur_li_but = Ref.getitem(
                                    self.question_info['ans'],
                                    self.li_but_count)
                                with fullp.insert() as liinsert:
                                    # Create Toggle button with Group_id
                                    litb =\
                                        ToggleButton(but_name=self.cur_li_but,
                                    ...
                                    # Create label to display the answer
                                    lilb =\
                                        Label(text=self.cur_li_but,
                                              center_x=litb.center_x,
                                              font_size=font_size * 3 / 4,
                                              y=(litb.top +
                                                 (0.5 * min_marg_dist)),
                                              # text_size=(2*min_marg_dist,
                                              #            None),
                                              save_log=False)
                                # Append the temp list of buttons
                            ...
                            with fullp.insert():
                                ...
                                lilbf =\
                                    Label(text=self.question,
                                          ...
                                          font_size=font_size,
                                          save_log=False)
                            # Update the height of the rectangle, buttom, and
                            # height_count
                            lirec.height = lilbf.height
                            self.new_bottom = lirec.top
                            self.height_count = self.height_count + \
                                (lirec.top - self.current_bottom)
                        ...
                        with Elif(self.q_type == "CA"):
                            ...
                            with Loop(Ref(len,
                                ...
                                with fullp.insert() as cainsert:
                                    ...
                                    # Create Toggle button with Group_id
                                    cavtb =\
                                        ToggleButton(but_name=self.cur_ca_but,
                                                     left=(fl.left +
                                                           min_marg_dist),
                                ...
                                tilb =\
                                    Label(text=self.question,
                                          ...
                                          text_size=((fl.width - 4 *
                                                      min_marg_dist), None),
                                          save_log=False)
                            # Might change to [[tiinsert.first]]
                            self.TI_refs += [tiinsert.first]
                            self.questions +=\
                                [{"answers_index": Ref(len, self.TI_refs) - 1,
                                  "question": self.question, "type": "TI",
                                  "index": self.fullcount}]
                            tirec.height = tilb.height
                            self.new_bottom = tilb.top
                            self.height_count =\
                                (self.height_count + tilb.height + tif.height +
                                    (2 * min_marg_dist))

                        with Elif(self.q_type == "CT"):
                            with fullp.insert() as ctinsert:
                                slf =\
                                    Slider(left=fl.left + min_marg_dist,
                                           width=fl.width - 2 * min_marg_dist,
                                           min=self.question_info[
                                             'min'],
                                           ...
                                           save_log=False)
                                slminlbf =\
                                    Label(text=self.question_info['ans'][0],
                                          left=slf.left,
                                          bottom=slf.top - 1.7 * min_marg_dist,
                                          font_size=font_size * 3 / 4,
                                          save_log=False)
                                slmidlbf =\
                                    Label(text=self.question_info['ans'][1],
                                          center_x=slf.center_x,
                                          bottom=slf.top - 1.7 * min_marg_dist,
                                          font_size=font_size * 3 / 4,
                                          save_log=False)
                                slmaxlbf =\
                                    Label(text=self.question_info['ans'][2],
                                          right=slf.right,
                                          bottom=slf.top - 1.7 * min_marg_dist,
                                          font_size=font_size * 3 / 4,
                                          save_log=False)
                                ...
                                sllbf =\
                                    Label(text=self.question,
                                          ...
                                          font_size=font_size,
                                          save_log=False)
                            # MIGHT HAVE TO CHANGE THIS to [[ctinsert.first]]
                            self.CT_refs += [ctinsert.first]
                            self.questions +=\
                                [{"answers_index": Ref(len, self.CT_refs) - 1,
                                  "question": self.question, "type": "CT",
                                  "index": self.fullcount}]
                            slrec.height = sllbf.height
                            self.new_bottom = sllbf.top
                            self.height_count =\
                                (self.height_count + slf.height +
                                 slminlbf.height + sllbf.height + 0.3 *
                                 min_marg_dist)
                        ...
                        with Elif(self.q_type == "TITLE"):
                            with fullp.insert():
                                fulltitlb =\
                                    Label(text=self.question,
                                          bottom=(self.new_bottom + 2 *
                                                  min_marg_dist),
                                          font_size=font_size * 1.5,
                                          center_x=self.x + self.width / 2)
                            self.height_count =\
                                (self.height_count +
                                    (fulltitlb.top - self.new_bottom) + 4 *
                                    min_marg_dist)
    ...
    with UntilDone():
        with ButtonPress():
            Rectangle(color="GRAY", left=sv.left,
                      height=self.button_area, width=self.width, y=self.y)
        ...
        with Loop(self.questions) as question:
                ...
                self.results +=\
                    [{"question": question.current['question'],
                      ...
                      "answers":[{"ans": "text_input_value",
                                  "value": self.TI_refs[question.current[
                                                    'answers_index']].text}]}]
                ...
                self.results +=\
                    [{"question": question.current['question'],
                      ...
                      "answers":[{"ans": "slider_value",
                                  "value": self.CT_refs[question.current[
                                    'answers_index']].value}]}]
            with Elif(question.current['type'] == "MC"):
                self.mc_temp_answers_list = []
                with Loop(self.MC_refs[question.current[
                                            'answers_index']]) as mc_loop_ref:
                    self.mc_temp_answers_list +=\
                        [{"ans": mc_loop_ref.current.but_name,
            ...
            with Elif(question.current['type'] == "CA"):
                ...
                with Loop(self.CA_refs[question.current[
                                            'answers_index']]) as ca_loop_ref:
                    self.ca_temp_answers_list +=\
                        [{"ans": ca_loop_ref.current.but_name,
            ...
            with Elif(question.current['type'] == "LI"):
                ...
                with Loop(self.LI_refs[question.current[
                                            'answers_index']]) as li_loop_ref:
                    self.li_temp_answers_list +=\
                        [{"ans": li_loop_ref.current.but_name,
...
if __name__ == "__main__":
    ...
    bob = [{'type': "TITLE",
           ...
           {'type': "LI",
            ...
            'ans': ['That is the Question.', 'To be is to do.',
                    'To do is to be.'],
            'group_id':'second_li_question'},
           {'type': "CT",
            ...
            'min':-5},
           {'type': "CT",
            'question': ("Where left is Too much, and right is too little, "
                         "and middle is just right,"
           ...
           {'type': "TI",
            'question': ("If you could do anything in the "
           ...
           {'type': "MC",
            ...
            'ans': ['Eternal Sunshine of the Spotless Mind',
                    'Yes Man', 'A Series of Unfortunate Events',
    ...
    with Parallel():
        tt = Questionnaire(loq=bob, height=exp.screen.height,
