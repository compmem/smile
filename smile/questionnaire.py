from video import WidgetState, UpdateWidget, Label, ToggleButton, FloatLayout, ButtonPress, Button, Rectangle, TextInput, Slider
from state import Loop, Parallel, If, Elif, Else, Serial, Func, UntilDone, Log, Debug, Subroutine
from ref import Ref
...
def Questionnaire(self,
    ...
    """A state that allows you to present a number of questions to a Participant

    Passing in a list of dictionairies with different keys will allow you to
    display the questions in the order that they show up in the list. Below you
    will see the parameters you can pass into this state, along with what keys
    you need for dictionaries of each type of equestion that can be displayed
    with this state.

    Parameters
    ----------
    loq : list of dictionaries
        A list of dictionaries that contain keys associated with the types of
        questions in the order you would like them displayed. You can also
        display a Title anywhere in the list of questions, or in multiple places,
        to title a section of questions.
    height : integer
    ...
    results : list of dictionaries
        *self.results* is the list of dictionaries containing the list of
        results for each of the questions displayed. The keys for each result
        are *question*, *answers*, *type*, and *index*. *question* is a string,
        *answers* is a list of answers depending on the *type* which is also a
        string, and *index* is where it appeared in the list of questions.

    Types of Questions
    ------------------
    In your loq list of dictionaries, you always need a *type* key. Into each
    *type* you pass in a string representing the type of question that you want
    ...
    CA : MULTIPLE CHOICE Choose All That Apply
        ...
        "ans" : list
            A list of strings, displayed in the order of ans[0] at the top and
            ans[-1] at the bottom, where each string is an answer
    ...
    TI : Text Input
        ...
        "question" : string
            Question to be displayed.
        "multiline" : integer
            A number, at least 1, that is the number of lines tall the text
            input box is.
    ...
    CT : Continuum Answer
        ...
        "question" : string
            Question to be displayed.
        "ans" : list
            A list of 3 strings, can be empty strings. ans[0] is to indicate
            what the far left value is, ans[1] is what the middle value is,
            and ans[2] is what the far right value is.
    ...
    LI : Likert
        ...
        "question" : string
            Question to be displayed.
        "ans" : list
            A list of 2 to 10 strings. Can be empty strings. ans[i] is displayed
            above the coorisponding radio button.
        "group_id" : string
            A unique string with no spaces to identify this question.
    ...
    with ScrollView(width=self.width, height=self.height-self.button_area, x=self.x, y=self.y+self.button_area) as sv:
        with FloatLayout(size=sv.size, pos=sv.pos) as fl:
            with Parallel() as fullp:
                rcf = Rectangle(color=(0.3, 0.3, 0.3, 1.0),top=fl.top, left=fl.left,
                ...
                with Serial():
                    with Loop(Ref(len, self.loq)) as lp:
                        ...
                        self.question_info_default = {"type":"TI","question":"","multiline":1,
                                                      "ans":[],"group_id":"","max":10, "min":-10}
                        qfc = Func(update_dict,self.question_info_default, self.row)
                        ...
                        with If(self.q_type == "MC"):
                            ...
                            self.tog_but_count = Ref(len, self.question_info['ans'])
                            ...
                            with Loop(Ref(len, self.question_info['ans'])) as mcvlp:
                                ...
                                self.cur_tog_but = Ref.getitem(self.question_info['ans'], self.tog_but_count)
                                with fullp.insert() as tbinsert:
                                    ...
                                    mcvlb = Label(text=self.cur_tog_but,
                                                  ...
                                                  text_size=(fl.width - 4*min_marg_dist - 2.5*min_marg_dist, None),
                                                  save_log=False)
                            ...
                            self.questions += [{"answers_index":Ref(len,self.MC_refs) - 1, "question":self.question, "type":"MC",
                                                "index":self.fullcount}]
                            #Add the question to the window aswell.
                            #Add the question to the window aswell.
                            with fullp.insert():
                                mcrec = Rectangle(color=(0.2, 0.2, 0.2, 1.0), left=fl.left+1.5*min_marg_dist,
                                                  width=fl.width - 3.0*min_marg_dist,
                                                  bottom=self.new_bottom+min_marg_dist,
                                                  save_log=False)
                                mclbf = Label(text=self.question, left=fl.left+2*min_marg_dist,
                                              bottom=self.new_bottom+min_marg_dist,
                            ...
                            self.height_count = self.height_count + (mcrec.top - self.current_bottom)
                        ...
                        with If(self.q_type == "LI"):
                            ...
                            with Loop(Ref(len, self.question_info['ans'])) as lilp:
                                ...
                                #Get the current answer
                                self.cur_li_but = Ref.getitem(self.question_info['ans'], self.li_but_count)
                            ...
                            self.questions += [{"answers_index":Ref(len,self.MC_refs) - 1, "question":self.question, "type":"LI",
                                                "index":self.fullcount}]
                            #Add the question to the window aswell.
                            #Add the question to the window aswell.
                            with fullp.insert():
                                lirec = Rectangle(color=(0.2, 0.2, 0.2, 1.0), left=fl.left+1.5*min_marg_dist,
                                                  width=fl.width - 3.0*min_marg_dist,
                                                  bottom=self.new_bottom+min_marg_dist,
                                                  save_log=False)
                                lilbf = Label(text=self.question, left=fl.left+2*min_marg_dist,
                                              bottom=self.new_bottom+min_marg_dist,
                                              text_size=(fl.width - 4*min_marg_dist, None),
                                              font_size=font_size,
                                              save_log=False)
                            ...
                            self.height_count = self.height_count + (lirec.top - self.current_bottom)
                        ...
                        with Elif(self.q_type == "CA"):
                            ...
                            with Loop(Ref(len, self.question_info['ans'])) as cavlp:
                                ...
                                #Get the current answer
                                self.cur_ca_but = Ref.getitem(self.question_info['ans'], self.ca_but_count)
                            ...
                            self.questions += [{"answers_index":Ref(len,self.CA_refs) - 1, "question":self.question, "type":"CA",
                                                "index":self.fullcount}]

                            #Add the question to the window aswell.
                            with fullp.insert():
                                carec = Rectangle(color=(0.2, 0.2, 0.2, 1.0), left=fl.left+1.5*min_marg_dist,
                                                  width=fl.width - 3.0*min_marg_dist,
                                                  bottom=self.new_bottom+min_marg_dist,
                                                  save_log=False)
                                calbf = Label(text=self.question, left=fl.left+2*min_marg_dist,
                                              bottom=self.new_bottom+min_marg_dist,
                                              text_size=(fl.width - 4*min_marg_dist, None),
                                              font_size=font_size,
                                              save_log=False)
                            ...
                            self.height_count = self.height_count + (carec.top - self.current_bottom)
                        ...
                        with Elif(self.q_type == "TI"):
                            ...
                            with fullp.insert() as tiinsert:
                                ...
                                tirec = Rectangle(color=(0.2, 0.2, 0.2, 1.0), left=fl.left+1.5*min_marg_dist,
                                                  width=fl.width - 3*min_marg_dist,
                                                  bottom=tif.top+min_marg_dist,
                                                  save_log=False)
                                tilb = Label(text=self.question, left=fl.left+2*min_marg_dist,
                                             bottom=tif.top+min_marg_dist, font_size=font_size,
                                             text_size=(fl.width- 4*min_marg_dist, None),
                                             save_log=False)
                            self.TI_refs += [tiinsert.first] # Might change to [[tiinsert.first]]
                            self.questions += [{"answers_index": Ref(len,self.TI_refs) - 1, "question":self.question, "type":"TI",
                                                "index":self.fullcount}]
                        ...
                        with Elif(self.q_type == "CT"):
                            with fullp.insert() as ctinsert:
                                slf = Slider(left=fl.left + min_marg_dist, width=fl.width - 2*min_marg_dist,
                                             min=self.question_info['min'], max=self.question_info['max'],
                                             bottom=self.new_bottom+min_marg_dist,
                                             save_log=False)
                                slminlbf = Label(text=self.question_info['ans'][0], left=slf.left,
                                                 bottom=slf.top-1.7*min_marg_dist,font_size=font_size*3/4,
                                                 save_log=False)
                                slmidlbf = Label(text=self.question_info['ans'][1], center_x=slf.center_x,
                                                 bottom=slf.top-1.7*min_marg_dist, font_size=font_size*3/4,
                                                 save_log=False)
                                slmaxlbf = Label(text=self.question_info['ans'][2], right=slf.right,
                                                 bottom=slf.top-1.7*min_marg_dist, font_size=font_size*3/4,
                                                 save_log=False)
                                slrec = Rectangle(color=(0.2, 0.2, 0.2, 1.0), left=fl.left+1.5*min_marg_dist,
                                                  width=fl.width - 3*min_marg_dist,
                                                  bottom=slminlbf.top+min_marg_dist,
                                                  save_log=False)
                                sllbf = Label(text=self.question, left=fl.left+2*min_marg_dist,
                                              bottom=slminlbf.top + min_marg_dist,
                                              text_size=(fl.width - 4*min_marg_dist, None),
                                              font_size=font_size,
                                              save_log=False)
                            self.CT_refs += [ctinsert.first] # MIGHT HAVE TO CHANGE THIS to [[ctinsert.first]]
                            self.questions += [{"answers_index": Ref(len,self.CT_refs) - 1, "question":self.question, "type":"CT",
                                                "index":self.fullcount}]
                        ...
                        with Elif(self.q_type == "TITLE"):
                            ...
                            self.height_count = self.height_count + (fulltitlb.top - self.new_bottom) + 4*min_marg_dist
    ...
    with UntilDone():
        with ButtonPress():
            Rectangle(color="GRAY", left=sv.left, height=self.button_area, width=self.width, y=self.y)
            Button(text="Continue", left = self.x+.5*min_marg_dist, top=sv.bottom-.25*min_marg_dist, height=self.button_area*.75)
        self.group_counter = 0
        self.results = []

        with Loop(self.questions) as question:
            with If(question.current['type'] == "TI"):
                self.results += [{"question":question.current['question'], "type":"TI", "index":question.current['index'],
                                  "answers":[{"ans":"text_input_value", "value":self.TI_refs[question.current['answers_index']].text}]}]
            with Elif(question.current['type'] == "CT"):
                self.results += [{"question":question.current['question'], "type":"CT", "index":question.current['index'],
                                  "answers":[{"ans":"slider_value", "value":self.CT_refs[question.current['answers_index']].value}]}]
            with Elif(question.current['type'] == "MC"):
                ...
                self.results += [{"question":question.current['question'], "type":"MC", "index":question.current['index'],
            ...
            with Elif(question.current['type'] == "CA"):
                ...
                self.results += [{"question":question.current['question'], "type":"CA", "index":question.current['index'],
            ...
            with Elif(question.current['type'] == "LI"):
                ...
                self.results += [{"question":question.current['question'], "type":"LI", "index":question.current['index'],
...
if __name__ == "__main__":
    ...
    bob = [{'type':"TITLE",
           ...
           {'type':"LI",
            'question':"be be?",
            'ans':['Fa Fa','Me Me','Re Re','Do Do','So So', 'La La','Ti Ti', 'Do Do', 'Lordy', 'Bordy'],
            'group_id':'second_li_question'},
           {'type':"CT",
            'question':"From No moo, to Much Moo, how cow are you?",
            'ans':['NO MOO','SOME MOO','MUCH MOO'],
            'max':5,
            'min':-5},
           {'type':"CT",
            'question':"Where left is least and right is most, how much of a does it cost to buy a car?",
            'ans':['','',''],
            'max':10,
            'min':-10},
           {'type':"TI",
            'question':"CHOSE THE SECOND ONE!",
            'ans':['Not this one','This one','Not this one'],
            'group_id':'second_ans_question'},
           {'type':"TI",
            'question':"If you were a woodchuck for a day, would you chuck some wood?",
            'multiline':1},
           {'type':"TI",
            'question':"Tell me about your life as a Woodchuck",
            'multiline':3},
           {'type':"TI",
            'question':"For the Most money in the world, write a memoire about your life.",
            'multiline':3},
           {'type':"MC",
            'question':"Choose the Third Answer please!",
            'ans':['Not this one','Not This one','This one', 'BANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLLBANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLLBANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLLBANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLLBANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLLBANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLLBANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLLBANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLLBANG BANG BANG GOES THE TROLLY, CLANG CLANG CLANG GOES THE BELLLLLLLLL'],
            'group_id':'third_ans_question'},
           {'type':"CA",
            'question':"Choose the all that apply!",
            'ans':['Not this one','Not This one','This one'],
